...
(See [complete-ops.py IPO](#complete-ops.py) above for the script-level detail.)
...
/complete
  ...
  OUTPUT: feature.yaml phase=complete, feature-index archived, final-summary.md
```

---

### Journey B: `express` Track — Single-Session Accelerated Planning

```
INPUT: User idea + explicit express track selection
  │
  ▼
/init-feature (express track)
  OUTPUT: feature.yaml track=express + 2 branches
  │
  ▼
/expressplan
  INPUT:  feature.yaml, lifecycle.yaml, lens-bmad-skill-registry.json
  PROCESS: Step 1 — bmad-lens-bmad-skill → quickplan-ops.py (all planning in one session)
           Step 2 — adversarial review (fail=halt, no advance)
           Step 3 — feature.yaml phase=expressplan-complete; signal /finalizeplan
  OUTPUT: All planning artifacts (product-brief, prd, ux-design, architecture, epics, stories)
          expressplan-adversarial-review.md
  │
  ▼
/finalizeplan (abbreviated — artifacts already generated)
  INPUT:  expressplan artifacts (already staged)
  PROCESS: verify bundle integrity, open final PR to main
  OUTPUT: PRs + feature.yaml milestone=dev-ready
  │
  ▼
/dev → /complete (same as full track)
```

---

### Journey C: `hotfix` Track — Urgent Fix

```
INPUT: User hotfix request
  │
  ▼
/init-feature (hotfix track)
  OUTPUT: feature.yaml track=hotfix + 2 branches
  │
  ▼
/techplan (only phase in hotfix track)
  INPUT:  feature.yaml, architecture template (minimal), constitution (informational gate only)
  PROCESS: winston (Architect) → minimal architecture; adversarial review (constitution gate is informational)
  OUTPUT: architecture.md (minimal) → feature.yaml phase=techplan-complete → auto-advance to /finalizeplan
  │
  ▼
/finalizeplan → /dev → /complete (same pattern, smaller scope)
```

---

### Journey D: Migration (v3 → v4)

```
INPUT: Existing v3 legacy branches (pattern: {domain}-{service}-{feature}[-{milestone}])
  │
  ▼
/migrate scan
  INPUT:  git branches (control repo), docs/ folder (legacy paths)
  OUTPUT: Scan report (list of features found, their docs, migration plan)
  │
  ▼
/migrate dry-run
  INPUT:  scan results, feature-index.yaml (duplicate detection)
  OUTPUT: Dry-run report (what would be created, what conflicts exist) — NO file writes
  │
  ▼  [user reviews dry-run, confirms]
/migrate migrate
  INPUT:  dry-run results (approved), lifecycle.yaml (schema_version=4)
  PROCESS: For each legacy feature:
           1. Conflict check (existing feature.yaml?)
           2. Create {featureId} + {featureId}-plan branches
           3. Mirror legacy docs to governance migrations/ folder
           4. Create feature.yaml + feature-index.yaml entry
  OUTPUT: New 2-branch topology, governance entries, migration proof docs
  │
  ▼
/migrate verify
  INPUT:  migration output
  OUTPUT: Verification report (all features migrated, all docs present)
  │
  ▼  [user reviews, approves cleanup]
/migrate cleanup
  INPUT:  cleanup-approval.yaml (required)
  OUTPUT: cleanup-receipt.yaml (written first), then legacy branches deleted
```

---

### Summary: Data Flow by Repository

```
GOVERNANCE REPO (always main)
  Written by:  init-feature, feature-yaml, complete, migrate, log-problem, target-repo,
               git-orchestration (publish-to-governance), move-feature, split-feature
  Read by:     next, switch, sensing, constitution, adversarial-review, dashboard,
               all phase conductors (via feature.yaml), lens.agent.md

CONTROL REPO (feature branches)
  Written by:  git-orchestration (commits, branches, merges), all phase conductors (artifacts),
               finalizeplan (epics/stories bundle), migrate (new branches)
  Read by:     git-state, sensing (branch scan), preflight, next-ops.py

TARGET REPO (code)
  Written by:  git-orchestration (prepare-dev-branch), dev phase (via delegated subagents)
  Read by:     target-repo (inventory), git-state (via target repo path)

MODULE SOURCE (lens-work/ itself)
  Written by:  never at runtime (read-only source)
  Read by:     all skills (lifecycle.yaml, templates, constitutions-template, agent manifest)
```
