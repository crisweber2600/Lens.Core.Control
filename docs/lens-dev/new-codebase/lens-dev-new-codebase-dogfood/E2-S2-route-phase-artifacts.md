...
1. Document the routing rules: planning before FinalizePlan → `{featureId}-plan`; FinalizePlan step 1/2 → `{featureId}-plan` then merged to `{featureId}`; FinalizePlan step 3 bundle → `{featureId}` (or `{featureId}-dev`); Dev → target repo only.
...
- [ ] FinalizePlan step 3 bundle routed to `{featureId}` (or `{featureId}-dev`).
