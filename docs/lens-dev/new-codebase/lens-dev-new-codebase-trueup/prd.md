...
| `new-feature` | `finalizeplan-complete` | Primary `create` command absent from the checked-in new-codebase source script under audit (any successful in-session run would have to come from a RELEASE/runtime copy or other non-source artifact) |
...
**new-feature (init-feature-ops.py):** The old-codebase discovery docs and source both describe `bmad-lens-init-feature` as a first-class planning surface with `create`, `fetch-context`, `read-context`, published prompts, and the `references/auto-context-pull.md` and `references/init-feature.md` guidance files. In the checked-in new-codebase source tree audited for this PR, the `init-feature-ops.py` script (798 lines) exposes only `create-domain` and `create-service`. The `create` subcommand — which performs canonical featureId construction, lifecycle track resolution, feature.yaml authoring, feature-index.yaml registration, control-repo 2-branch creation, and planning PR creation — is absent from that source file. The following are also absent from the audited source tree: `fetch-context`, `read-context`, and all supporting functions (`resolve_feature_identity`, `make_feature_yaml`, `load_lifecycle`, `resolve_start_phase`, etc.). If research observed `create` running successfully in-session, that evidence should be interpreted as coming from a RELEASE/runtime module copy or other environment-specific artifact, not from the checked-in new-codebase source tree described here. Because BusinessPlan and other planning flows rely on Auto-Context Pull, absence of `fetch-context` and `read-context` in the source tree is a functional regression, not just a missing convenience command.
