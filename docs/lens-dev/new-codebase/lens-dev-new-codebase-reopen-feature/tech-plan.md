...
3. Validate `--to-phase` is valid for track and non-terminal.
4. Update `feature.yaml`:
   - `phase = to-phase`
   - `status = active` when previously archived/complete
   - remove `completed_at` if present
   - append `phase_transitions` entry
5. Sync `feature-index.yaml` via existing sync helper.

## Validation

- Success test: complete+archived feature reopens to expressplan and index status unarchives.
