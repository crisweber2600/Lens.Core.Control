...
key_decisions:
  ...
  - Record quickdev and commit evidence in generated versioned quickdev artifacts under `quickdev/` that publish to governance.
  - Commit directly to active feature branches; otherwise create a branch and PR through standard Lens git flow.
  - Treat feature-associated control-repo docs as the default non-source scope and require a documented override before broader documentation or packaging changes.
open_questions: []
depends_on:
  - business-plan.md
  - tech-plan.md
...
Ship a public `lens-quickdev` conductor that lets users provide a scoped implementation ask during dev-ready execution, delegates code changes to existing BMAD quick-dev behavior, and records quickdev plus commit evidence in generated versioned evidence documents under `quickdev/` that publish to governance.
...
- Update feature-associated control-repo docs when the command contract changes; if broader control-repo or packaging/discovery work becomes necessary and materially expands scope, warn the user and document any approved override before editing those surfaces.
...
- Given broader non-source updates are discovered, when the implementation would expand beyond feature docs, then the scope-creep warning and any approved override are recorded before those surfaces change.
...
- Treat target repo registration as a FinalizePlan-owned prerequisite, resolve it from `feature.yaml.target_repos`, and block when absent at runtime.
- Inspect target repo branch and dirty status before any implementation.
- Generate `quickdev/quickdev-[summaryofrequeststub]-vNNN.md` with request, assessment, assumptions, validation plan, and implementation plan.
...
- Given a valid target repo, when assessment completes, then a new versioned quickdev artifact exists under `feature.yaml.docs.path/quickdev/`.
...
- Update the generated versioned quickdev evidence file with branch, commit hash, changed files, validation, PR link when available, and final outcome.
- Publish the generated versioned quickdev evidence file to governance.

Acceptance criteria:
- Given implementation changes are made, when the wrapper completes, then target repo has a non-empty commit hash recorded in the generated versioned quickdev artifact.
- Given validation runs, when the wrapper completes, then validation command and result are recorded in the generated versioned quickdev artifact.
...
- Given the quickdev evidence is finalized, when governance publication runs, then the same versioned file is present under `feature.yaml.docs.governance_docs_path/quickdev/`.
...
- Add tests for redirect-only prompt, registration, dev-ready gate, missing target repo, generated versioned evidence docs, governance publication, branch policy, validation failure, rerun versioning, and no-op behavior.
...
- Given the same ask is run twice, when the second run completes, then it creates a new versioned quickdev artifact instead of overwriting the first.
...
- Codebase assessment, implementation plan, validation, branch, commit, and PR evidence are captured in generated versioned quickdev docs under `quickdev/`.
- Generated quickdev docs are published to governance.
- Any approved expansion beyond feature-associated control-repo docs is documented as a scope override.
