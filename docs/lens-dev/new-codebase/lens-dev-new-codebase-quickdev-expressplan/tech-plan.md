...
key_decisions:
  ...
  - Store all quickdev and commit evidence in one generated quickdev-[summaryofrequeststub].md file.
open_questions:
  - Whether command registration belongs only in module.yaml/module-help.csv or also requires setup.py prompt metadata in this slice.
depends_on:
  - business-plan.md
blocks: []
...
If release packaging expects prompt metadata outside the source module, add that as a separately reviewed acceptance item rather than silently editing control-repo prompt mirrors.
...
5. Resolve the target repo from `feature.yaml.target_repos[0].local_path`. If absent, stop with a clear target-repo blocker and do not guess.
6. Resolve docs path from `feature.yaml.docs.path` and governance docs path from `feature.yaml.docs.governance_docs_path`; create the staged docs folder when missing.
7. Generate the evidence file name from the request as `quickdev-[summaryofrequeststub].md`:
  - lowercase the request summary
  - keep the first meaningful 4-8 words
  - replace non-alphanumeric runs with `-`
  - trim leading and trailing `-`
  - append a stable short suffix only if the file already exists for a different run
...
9. Write or refresh `quickdev-[summaryofrequeststub].md` with request, assessment, assumptions, validation plan, and implementation plan.
...
17. Update `quickdev-[summaryofrequeststub].md` with branch, commit hash, changed files, validation command, validation result, PR URL when created, and final outcome.
18. Publish the same quickdev evidence file to `feature.yaml.docs.governance_docs_path` through the sanctioned Lens publication path, then commit/push the governance publication.

## Artifact Contracts

### `quickdev-[summaryofrequeststub].md`
...
- Evidence test: successful run writes one generated `quickdev-[summaryofrequeststub].md` file under `feature.yaml.docs.path` and publishes it to `feature.yaml.docs.governance_docs_path`.
...
| Target repo cannot be resolved for older features | Medium | Block clearly and require FinalizePlan or feature metadata to add `target_repos`. |
